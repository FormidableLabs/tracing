...
* `ignoreExtensions` (`Array<string>`): A set of file extensions (e.g., `.map` or `.graphql`) to skip tracing on.  These are added to our built-in extensions to skip of `.json` and `.node`.
